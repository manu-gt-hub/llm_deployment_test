...
def get_trading_view_opinion(symbol):
    print(f"📊 Fetching TradingView opinion for {symbol}...")
    ...
    try:
        ...
        for div in opinion_divs:
            if 'Summary' in div.text:
                ...
                print(f"✅ [{symbol}] Opinion fetched: {result}")
                return result

        print(f"⚠️ [{symbol}] No summary found.")

    except Exception as e:
        print(f"❌ Error fetching opinion for {symbol}: {e}")
        return f"error: {e}"
